...
Now we are ready to run backtrack search! A final optimization is to choose the node with the smallest domain at each step of the search. This will cause failures to occur quicker, and we will be able to prune more paths from the search tree. This can be very important, because in a 9 $\times$ 9 grid there could be 9$\exp$81 possible assignments, which would take literally forever to try!
```
